...
export class MemoryKeyStore implements KeyStore {
  ...
  public storeKeys(keys: EncryptedKey[]) {
    ...
    const keysMetadata = keys.map((encryptedKey: EncryptedKey) => {
      ...
      return getKeyMetadata(this.keyStore[encryptedKey.id] as EncryptedKey);
    });
  ...
  public updateKeys(keys: EncryptedKey[]) {
    ...
    const keysMetadata = keys.map((encryptedKey: EncryptedKey) => {
      ...
      return getKeyMetadata(this.keyStore[id] as EncryptedKey);
    });
  ...
  public removeKey(id: string) {
    ...
    const metadata: KeyMetadata = getKeyMetadata(
      this.keyStore[id] as EncryptedKey
    );
